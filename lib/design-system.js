...
export const LAYOUT = {
  ...
  pad: {
    top:    72,
    right:  80,
    bottom: 48,   // reduced to pull attribution closer to canvas bottom
    left:   80,
  },
  ...
  chartAreaBottom() {
    // Extra 52px reserves space for multi-line category labels below bars
    return CANVAS.height - this.pad.bottom - ATTRIBUTION.height - 52;
  },
  chartAreaLeft() {
...
export const ATTRIBUTION = {
  ...
  source:   "LOUISIANA STARTUP REPORT 2026",
...
// ------------------------------------------------------------
// DARK MODE overrides per palette
// Each entry replaces bg/soft/text/textMid for dark canvas.
// mid/primary/deep are kept as-is (they look good on dark).
// ------------------------------------------------------------

const DARK_OVERRIDES = {
  indigo: { bg: "#0d0d20", soft: "#1e1e3a", text: "#e8e8f8", textMid: "#9090b8" },
  blue:   { bg: "#090f1e", soft: "#12213a", text: "#e8eef8", textMid: "#7a9ec8" },
  teal:   { bg: "#07161a", soft: "#0f2a2e", text: "#e4f4f4", textMid: "#6aabaa" },
  green:  { bg: "#081408", soft: "#102610", text: "#e4f4e4", textMid: "#76aa7a" },
  amber:  { bg: "#1a1206", soft: "#2e2008", text: "#faf4e4", textMid: "#c8a460" },
  rose:   { bg: "#1a0a0c", soft: "#2e1214", text: "#f8e8ea", textMid: "#c07888" },
};
...
// ------------------------------------------------------------
// HELPER — get dark-mode version of a palette
// ------------------------------------------------------------

export function getDarkPalette(key) {
  const base = getPalette(key);
  const overrides = DARK_OVERRIDES[key] || DARK_OVERRIDES.indigo;
