...
When your response contains data that would make a useful visualization, append a fenced code block tagged \`chart-config\` at the very end of your response (after all prose). The UI will strip it from display and offer a "Visualize" button.

Only include a chart block when the data genuinely benefits from visualization (distributions, comparisons, trends over time). Do not force charts on simple single-number answers.
